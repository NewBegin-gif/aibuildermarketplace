...
lang_flags = {'-en':'🇬🇧','-nl':'🇳🇱','-fr':'🇫🇷','-de':'🇩🇪','-es':'🇪🇸','-it':'🇮🇹','-pt':'🇵🇹','-pl':'🇵🇱','-sv':'🇸🇪','-ja':'🇯🇵','-ko':'🇰🇷','-du':'🇳🇱','-ge':'🇩🇪','-sp':'🇪🇸','-po':'🇵🇹'}
tool_icons = {'Kinsta':'🟢','Synthesia':'🎬','InVideo':'🎥','Replit':'💻','Bitvavo':'₿','Clay':'🎯','Murf':'🎙️','Other':'🔧','WP Rocket':'🚀','Rank Math':'📊'}
...
def get_tool(slug):
    s = slug.lower()
    for t in ['kinsta','synthesia','invideo','replit','bitvavo','clay','murf','wprocket','wp-rocket','rankmath','rank-math']:
        if s.startswith(t): return {'invideo':'InVideo','wprocket':'WP Rocket','wp-rocket':'WP Rocket','rankmath':'Rank Math','rank-math':'Rank Math'}.get(t, t.capitalize())
    return 'Other'
...
def _read_html(folder):
    try:
        with open("b2b/" + folder + "/index.html", encoding="utf-8") as fh:
            return fh.read()
    except Exception:
        return ""

def get_card_title(folder):
    """Pak <h1> uit artikel (target lang). Fallback: clean_title(slug)."""
    html = _read_html(folder)
    ...
    if m:
        ...
        if text:
            return text[:80]
    return clean_title(folder)

def get_card_desc(folder):
    """Pak eerste betekenisvolle <p> uit artikel (target lang). HTML-tags worden gestript."""
    html = _read_html(folder)
    # Match <p>...</p> met willekeurige inhoud (incl nested tags) - non-greedy
    for m in re.finditer(r"<p\b[^>]*>(.*?)</p>", html, re.DOTALL):
        text = re.sub(r"<[^>]+>", "", m.group(1))
        text = re.sub(r"\s+", " ", text).strip()
        if len(text) >= 50:
            return (text[:160].rstrip(",.;: ") + "...") if len(text) > 160 else text
    # Fallback: try unclosed <p>
    m = re.search(r"<p\b[^>]*>(.{60,400})", html, re.DOTALL)
    if m:
        text = re.sub(r"<[^>]+>", "", m.group(1))
        text = re.sub(r"\s+", " ", text).strip()
        if text:
            return text[:160] + "..."
    try:
        return get_desc(folder)
    except Exception:
        return ""

cards = [{'slug':f,'tool':get_tool(f),'type':get_type(f),'title':get_card_title(f),'desc':get_card_desc(f),'icon':tool_icons.get(get_tool(f),'🔧'),'color':tag_colors.get(get_type(f),'#374151'),'lang':get_lang(f)} for f in folders]
...
<body>
  ...
  <div class="filters-wrap"><div class="filters-inner">
    ...
    <button class="filter-btn" onclick="filterTool(this,'Rank Math')">📊 Rank Math</button>
    <button class="filter-btn" onclick="filterTool(this,'WP Rocket')">🚀 WP Rocket</button>
